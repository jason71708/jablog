...
Quick Sort 使用基準值 (pivot) 比對排序，並透過 [Recursion](./09-recursion.md) 的技巧，不斷將每個元素放到正確的位置上。
...
11. 1. 基準值與 `index = 10` 的元素比對：`7` 比較小，`pivotIndex` 需加 1 (`pivotIndex = 8`)，並與 `pivotIndex` 所在的元素進行交換位置 (Swap)。
...
12. 1. 基準值與 `index = 11` 的元素比對：`6` 比較小，`pivotIndex` 需加 1 (`pivotIndex = 9`)，並與 `pivotIndex` 所在的元素進行交換位置 (Swap)。
